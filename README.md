...
- RSS 候选按 Torra 订阅与唯一产物跨批次分组，保留逐集评分并要求多集产物覆盖的全部季集均为冠军；整组仅因关联订阅不存在而阻断时进入独立“待整理”，不占候选评分主列表。严格高分冠军继续按唯一 Torra 订阅精准提交。普通已识别资源可在人工确认后直接提交 qB：优先沿用唯一 Torra 订阅的八分类，没有订阅时电影固定进入 `10-电影`，电视剧只在 TMDB 地区、类型或语言可以唯一判定时进入对应分类。服务端复核下载器、分类目录、qB 重复任务和预览指纹；浏览器不能提交下载地址、保存目录或分类，分类冲突时不会猜测。
