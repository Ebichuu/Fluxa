...
- 首页只读 SQLite 模块摘要，冷启动不现场请求 Torra、qB、Symedia、Emby、RSS 或追更工作台；各模块独立刷新，缺少证据时不显示绿色正常，已确认数量与暂未确认对象分开表达。
- 第一阶段追更工作台统一展示本地写入、Torra、镜像同步、RSS 和定时任务状态；列表读取 SQLite 最后可靠快照并由后台每 60 秒单飞更新，容器重启与上游短暂离线不会阻塞首屏。Torra 只读订阅可直接进入追更与日历，Fluxa 本地台账仍保持独立边界。
...
- 私人 PT RSS 资源中心、本地全文搜索、订阅目标精确筛选、TMDB/IMDb 身份与有界历史回填；全库计数与默认 24 小时列表明确分开，普通“待识别”只统计已关联追更的资源，其余条目中性标记为“未关联追更”，旧 `/rss-library` 地址保持兼容。
- RSS 候选按 Torra 订阅与唯一产物跨批次分组，保留逐集评分并要求多集产物覆盖的全部季集均为冠军；整组仅因关联订阅不存在而阻断时进入独立“待整理”，不占候选评分主列表。严格高分冠军可在人工模式下预览并精准提交到 qB，保存目录和下载器映射必须从实时 Torra 订阅与服务端配置派生，浏览器不能提交这些字段；Torra 明确提供 qB 分类时才原样使用，缺失时省略分类，不从媒体分类、目录、标题或历史任务推断。自动模式仍保持关闭，必须先完成 qB、mover、p115client、Symedia 和 Emby 实机链路验收。
