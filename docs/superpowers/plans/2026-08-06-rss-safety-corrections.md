...
- 已新增失败测试并在 `27e0ae8` 上稳定复现四类问题。
- qB 同目标占用统一覆盖 `downloading/stalled/queued/paused`，并增加令牌签发后状态变化的执行前复核。
- 精准下载指纹已绑定完整下载地址的 SHA-256 摘要，地址变化时旧令牌返回 `RSS_EXACT_PREVIEW_STALE`，且不会调用 qB。
- `x264/x265/x266` 独立编码标记不再参与 `NxM` 季集解析，`1x03`、`1 x 03` 和既有 `S2026E70` 行为保持。
- 问题组阶段、原因码和摘要文案统一取自有效 `pipelineOutcome`，成员原始投影保持兼容。
- 目标测试通过；全量后端 766 项测试通过；TypeScript 检查和 Vite 生产构建通过。
- 安全扫描针对本次三个运行时文件通过，未发现敏感信息、注入或路径风险。
- 未修改数据库结构、API 请求格式、qB 状态归一化、执行模式或下载硬门禁；未连接真实外部写接口。
