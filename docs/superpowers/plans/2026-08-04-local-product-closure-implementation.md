...
- 批次 1–6 及收口补丁已完成。
- 日历、追更、首页与 RSS 定向回归通过；Python 全量回归 700 项、TypeScript 检查和生产构建通过。
- API 契约声明与实际路由均为 81 条；变更完整性、质量和安全检查通过。
- 隔离浏览器实机验收通过；全过程使用临时 SQLite 和虚构数据，未启动后台生产运行时，也未连接或触发 Torra、qB、115、Symedia、Emby 与真实 RSS 动作。

截至 2026-08-05 的“任务优先、缓存先行”增量：

- 首页主结果已收敛为今日入库、正在下载、等待自动恢复和需要处理；异常补充影响与单一推荐动作。
- 追更工作台新增 SQLite 持久快照、60 秒后台单飞刷新和 202 刷新请求；GET 只读缓存，失败保留旧值。
- 对账视图继续以规范目标展示，并可展开内部记录；全库记录与目标数同时表达。
- 首页资源摘要使用追更关联口径，站点原始 RSS 数量不再作为首页成果。
- v2 契约现为 81 条；Python 全量回归现为 700 项。
...
- 追更页把对账异常记录与规范目标分开显示；记录构成继续严格对账全库记录数，首页与详情统一使用“个目标”。
- 发现页白图、坏图和缺图统一显示标题首字，不再使用损坏图片图标。
- 首页资源摘要统一使用“追更待识别”。
- 当前生产包约 520.68 KB，Vite 体积提示作为非阻断后续优化项，本波次不引入代码拆分。
