...
- 已完成 A.1、A.3：健康证据、调度器心跳、RSS 匹配状态、导航壳和移动端管理菜单已接入。
- 已完成 B.1：首页统一读取 `/api/v2/home/summary`，不再由浏览器按接口是否返回成功拼装绿色结论。
- 已完成 B.2：`/api/v2/subscriptions/reconciliation` 只读返回 Fluxa/Torra 对账、履约和健康三维状态；标题只生成冲突候选，远端消失不删除本地意图。
- B.3 进行中：现有追更工作台已显示两套台账总数、五类差异、三维状态和 Torra-only 条目；v2 工作台已完成按媒体类型/关键词过滤后的 `limit + offset` 分页，独立页面组件拆分仍待后续提交。
- C.1 已完成本地账本第一段：schema 4 新增资源链、产物和事件表；任务链完整快照按状态指纹幂等落库，身份升级保留别名事件，产物冲突不会静默改绑，事件文本在写入前脱敏。
- C.2 已开始：任务中心已切换到只读 v2 任务链身份适配，动作边界保持不变。
- C.3 已完成第一段：任务卡展示健康与履约双状态、目标季集、chainId，以及每阶段状态、证据、来源、读取时间和原因；未引入新的外部写操作，qB 暂停/恢复仍需确认。
- C.4 已完成第一段：独立异常分类器按“需要处理 → 证据不足 → 等待 → 正常保护 → 正常”汇总任务状态；任务卡和阶段同时展示原因与下一步，低分保护不提供重试入口。
- 阶段 D 尚未开始，继续保持 Torra 推送和远端删除关闭。
...
6. 工作台按媒体类型/关键词过滤后使用 `limit + offset + nextOffset` 分页，避免一次返回数百 KB。
...
- 分类器统一返回 `recommendedAction`、`retryEligible` 和 `plannedRetryAt`；C.4 只展示建议，不创建真实重试动作。
- 正常保护固定提示“已保留低分源文件，可进入存储清理”，并保持 `retryEligible=false`。
