...
状态：已完成实现；2026-08-01 补齐资源中心全库口径、追更关联待识别与孤立候选待整理分流，进入全量验证与独立提交。
...
- 新资源、待识别、候选评分、追更洗版和待整理标签明确标注全库口径；默认 24 小时列表继续独立分页。
- 普通待识别只统计存在当前追更关系的身份缺口，其他 RSS 条目中性归入“未关联追更”；旧全库 `needs_review` 保持兼容。
- 全组均因关联订阅不存在而阻断的候选进入只读“待整理”，不占候选评分主列表，也不提供分析或下载动作。
- 阶段 B 继续负责可靠当前版本基线、跨批次冠军和真实升级决策，本阶段不自动下发。

键盘验收备注：当前浏览器自动化环境无法推进 Tab 焦点，因此键盘实机验收仍为“尚未确认”；没有证据表明是 Fluxa 本身故障，不将其记为功能失败或完成项。
