...
- 24 小时窗口：`2 / 6 / 12 / 24 小时`。
- 48 小时窗口：`2 / 6 / 12 / 24 / 36 / 48 小时`。

检查时间点仍允许通过订阅设置修改，但不得超过所选窗口。全局设置包括：
...
- 可编辑时间点列表；24 小时默认 `2, 6, 12, 24`，48 小时默认增加 `36, 48`。
- 最小间隔、每日上限、单批上限。
...
- 24 小时窗口的 `2/6/12/24`、48 小时窗口的 `2/6/12/24/36/48` 到期计算。
...
- 每条订阅可选择 24 或 48 小时追更洗版窗口，默认 48 小时，并从 2 小时检查点开始。
