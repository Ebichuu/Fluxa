...
追更洗版的数据触发优先依赖独立的私人 PT RSS 种子库设计：媒体控制中心保存最近 7 天 RSS 条目并做本地季集匹配，只有出现新候选时才唤醒 Torra；Torra 主动搜索降为有限兜底。详细设计见 `docs/superpowers/specs/2026-07-18-private-pt-rss-seed-library-design.md`。
...
- 不爬取 PT 站网页或实现第二套主动搜索器；独立 RSS 种子库只被动读取用户提供的标准 RSS。
...
- `torra_quality_schedule_json`，保存从 `baseline_ready_at` 起算的分钟偏移数组
- `torra_quality_default_window_hours`，只允许 `24` 或 `48`，默认 `48`
...
- `season_number`
- `title`
- `torra_quality_window_hours`，为空时继承全局值，只允许 `24` 或 `48`
- `payload_json`
- `created_at`
- `updated_at`
- `version`
...
追更洗版窗口从本集当前版本能够被 Torra 通过 Emby 读取的 `baseline_ready_at` 开始计算。全局默认窗口为 `48` 小时，每条订阅可以单独选择 `24` 或 `48` 小时。

RSS 种子库启用时，新条目可以在窗口内随时唤醒一次追更洗版。主动搜索只作为 RSS 漏报兜底，默认时间点为：

- 24 小时窗口：`12 / 24 小时`。
- 48 小时窗口：`12 / 24 / 48 小时`。

兜底时间点仍允许通过订阅设置修改。时间点按分钟保存，最小 30 分钟、严格递增、不得重复，也不得超过所选窗口，因此可以配置比默认 12 小时更早的检查。全局设置包括：

- 检查时间点列表。
- 默认追更洗版窗口，`24` 或 `48` 小时，默认 `48`。
- 最小搜索间隔，默认 60 分钟。
- 每日最大追更洗版次数。
- 单批最大处理数量。

单条订阅可以覆盖全局窗口和检查时间点。窗口一旦由本集 `baseline_ready_at` 建立，即固定计算 `observation_ends_at`；期间下载到更好版本不会重新计时，也不会延长窗口。恰好位于 24/48 小时截止点的最后一次检查允许执行，终态写入后如果仍未达到目标，再把当前集标记为 `observation_expired`。
...
这与 Torra 原界面的“选中分数更高”语义一致。追更洗版协调器不解析 Torra 主动搜索结果的质量高低、不自行计算质量分，也不修改 Torra 已有版本优先级；私人 RSS 种子库只解析用于本地搜索和季集粗匹配的轻量元数据。没有正分差候选时只记录“暂时没有更好版本”，继续等待新的 RSS 条目或下一个主动兜底点。Torra 仅接受任务不等于分析或下载完成，页面和日志必须区分“已提交”“执行中”和终态。

### 9.5 结束条件

- Torra 明确报告本次追更洗版完成，或当前版本已命中版本控制最高目标：`target_reached`。
- 当前集的 24/48 小时窗口到期：`observation_expired`，停止该集自动搜索并保留人工“再次检查”。
- 用户关闭或暂停：`paused`。
- 身份、版本控制或服务状态无法继续：`blocked`。

每一集单独建立观察窗口。窗口到期只关闭当前集的自动追更洗版，不关闭整条订阅。新一集首次下载成功并进入 Emby 后创建新的 24/48 小时观察窗口，不重置或重新开启其他集的状态。迁移已有订阅时不补建历史窗口，避免首次启用产生批量搜索。
...
- 总开关。
- 全局默认窗口：`24 小时` 或 `48 小时`，默认 `48 小时`。
- 可编辑主动兜底时间点列表；24 小时默认 `12, 24`，48 小时默认增加 `48`。
- RSS 种子库状态；启用后主动时间点是兜底，不是主要发现方式。
- 最小间隔、每日上限、单批上限。
...
追更洗版分析虽然不直接下载，但会让 Torra 搜索 PT 站点，因此使用 POST、独立分析闸门、冷却和幂等，并返回 `202 Accepted`、动作 ID 和 `Location` 轮询地址。候选下载同样返回 `202`，并继续要求会话、Origin、下载闸门、明确确认、幂等键和服务端复查。暂停、恢复和单条时间表覆盖通过同一个 PATCH 完成。时间表使用分钟偏移数组，最小 30 分钟、严格递增且不能超过 24/48 小时窗口。

动作查询必须区分本地动作状态与 Torra 外部 job 状态，并返回 `external_job_id` 的脱敏可审计值。`PATCH /quality-watch` 的 `window_hours` 只接受 `24` 或 `48`；超过窗口的时间点返回 `422`。读取和成功的 PATCH 返回 `200`；语义不合法返回 `422`；幂等或并发冲突返回 `409`；触发限频返回 `429`；上游不可达返回 `502`；对应功能闸门关闭返回 `503`。错误不能包装在 `200` 中。最终字段在实施时加入机器契约后冻结。
...
- RSS 新条目在窗口内即时唤醒，同一条目和观察单元只触发一次。
- 24 小时窗口的 `12/24`、48 小时窗口的 `12/24/48` 主动兜底计算。
- 24/48 小时截止点允许执行最后一次检查，完成后再关闭当前集窗口。
- 更好版本下载后不延长窗口；当前集到期关闭，下一集建立新窗口。
...
- 每条订阅可选择 24 或 48 小时追更洗版窗口，默认 48 小时；RSS 新条目即时唤醒，主动搜索只在 12/24 或 12/24/48 小时兜底。
- 当前集窗口不会因版本升级而延长；到期后不再自动搜索，下一集更新时重新建立窗口。
