...
状态：Python 单后端、PT 主线、SQLite 单台账与私人 RSS 种子库第一版已落地；Torra 追更洗版待实施
...
| RSS 种子索引 | 媒体控制中心 SQLite | 直接轮询 5～10 个私人 RSS，默认保留 7 天；不修改 Torra |
...
- 私人 RSS 来源、种子和动作读取接口已经进入 v2 契约；RSS → 订阅匹配和 Torra 追更洗版动作仍处于计划阶段。
...
- 26 条 `/api/v2` 机器契约；私人 RSS 页面已接入，网盘相关路由延期保留为 `client=false`。
...
- 私人 RSS 的 5～10 个真实脱敏站点夹具、429/指数退避和并发收集硬化。
- RSS 条目与订阅季集匹配，以及匹配后唤醒 Torra 的动作状态。
- Torra 追更洗版分析、按集观察和持久化幂等/冷却。
