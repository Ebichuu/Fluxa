...
| 秒传后交接 | 媒体控制中心 Python | 只把启用后新成功的 Torra 秒传文件逐个交给唯一 Symedia 归档任务；不扫描历史目录 |
...
- CloudDrive2 没有产生文件事件时，媒体控制中心可消费 Torra 新成功秒传 job 并调用 Symedia 官方单文件归档入口；首次启用建立水位、目标不唯一时拒绝执行，不改变 Torra 或 Symedia 的职责。
...
- 60 条 `/api/v2` 机器契约；私人 RSS 页面、阶段 6 追更洗版设置/动作路由、阶段 7 MoviePilot 人工备用路由和管理员运行时配置已接入，网盘相关路由延期保留为 `client=false`。
