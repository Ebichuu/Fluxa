...
状态：Python 单后端和 PT 单一主线已经落地；Telegram 网盘能力延期，等待 fnOS 实机验证
...
- MoviePilot 以后只在 Torra 明确无结果时补齐其他 PT 站点，不允许并行下载。
...
- 我的订阅：唯一台账和单条管理。
- 订阅设置：来源、定时和 Torra 单一主通道；不混入系统连接设置。
...
- MoviePilot：连接检查和订阅源码保留，未来用于其他 PT 站点补齐。
...
- MoviePilot 在 Torra 无结果后的 PT 补齐状态机。
