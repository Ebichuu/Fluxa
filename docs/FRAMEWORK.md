...
状态：Python 单后端和 PT 主线已落地；SQLite 单台账与 Torra 追更洗版待实施
...
后端
  ...
  ├─ 私人 PT RSS 种子库与 FTS5 本地索引（待实施）
  ├─ SQLite 订阅仓储与 Torra 追更洗版协调器（待实施）
  ├─ 四步任务链聚合
  └─ 活动日志、错误脱敏和安全动作
...
- SQLite、私人 RSS 种子库与 Torra 追更洗版的新 v2 接口仍处于设计状态，见 `docs/API_CONTRACT.md` 的计划章节；未实施前不属于当前契约。
...
- 私人 PT RSS 种子库、来源管理、FTS5 搜索和保留期清理。
