...
- 媒体控制中心自动触发 Torra 原本需要人工执行的追更洗版分析，只选择 Torra 返回的正分差最高候选；版本优先级仍由 Torra 判断。Torra 自身订阅完结后的能力称为“完结洗版”。
...
- SQLite 与 Torra 追更洗版的新 v2 接口仍处于设计状态，见 `docs/API_CONTRACT.md` 的计划章节；未实施前不属于当前契约。
...
- SQLite 保存订阅、配置、按集追更洗版观察、幂等动作、冷却、调度游标和迁移报告。
...
- Torra 追更洗版使用独立开关，默认关闭；每条订阅选择 24 或 48 小时窗口，默认 48 小时。检查点默认分别为 `2 / 6 / 12 / 24` 和 `2 / 6 / 12 / 24 / 36 / 48` 小时，版本升级不会延长窗口。
