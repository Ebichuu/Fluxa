...
状态：Python 单后端、PT 默认策略和 NasEmby 网盘安全接入已经落地；等待 fnOS 实机验证
...
| 网盘第二通道 | 115 + Telegram + HDHive / pansou | 统一页面、v2 安全 API、候选预览和单条转存保护已完成；真实动作待实机 |
...
网盘通道的已完成代码、剩余自动执行器和实机阶段见 `docs/CLOUD_ACQUISITION_PLAN.md`。
...
- `/api/v2/integrations`：115、Telegram、HDHive / pansou 和 MoviePilot 的脱敏配置/连接状态。
- `/api/v2/integrations/*`：Telegram 登录与频道、115 账号检查、HDHive 授权/配置/签到；每组使用独立闸门和冷却。
- `/api/v2/subscriptions/:id/cloud-policy`：订阅级网盘兜底开关。
- `/api/v2/acquisition/cloud/candidates`：只返回短期候选 ID 和脱敏摘要，不返回分享链接、密码或 Cookie。
- `/api/v2/acquisition/cloud/transfers`：单条确认转存；执行前复查 Torra、qB、Symedia 和 Emby，并使用幂等键与冷却。

### 已通过 v2 接入、原实现继续保留的核心能力

- 115：账号检查、候选转存和目标目录状态已接入；原监控、清理和助力继续保留关闭。
- Telegram：登录、退出和频道管理已接入。
- HDHive / pansou：授权、配置、签到、候选搜索和单条转存已接入。
...
- 全局与订阅级网盘开关、等待阈值、来源与自动选择策略。
- 115、Telegram、HDHive / pansou、MoviePilot 统一状态与管理入口。
- 脱敏网盘候选预览、单条安全转存、重复复查、幂等、冷却和审计。
- 13 条 `/api/v2` 机器契约及认证、Origin 和模拟回归测试。
...
- 自动兜底后台执行器；当前只计算和展示 `pt_waiting` / `cloud_allowed` 等状态，不会自动转存。
- 115 原批量监控、清理和助力的统一页面；高风险能力继续保留关闭，不能整体开放旧接口。
