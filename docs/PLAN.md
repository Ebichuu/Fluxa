...
状态：SQLite 单台账与私人 RSS 种子库第一版已落地；真实 RSS 和 Torra 追更洗版保持关闭
更新时间：2026-07-18
...
- [x] 完成 98 项 Python 回归测试，包括 SQLite、JSON 迁移、私人 RSS 解析/索引/API、保留核心接口和现有 PT 主链契约。
...
- [x] HTTP v2 契约扩展至 26 条，新增私人 RSS 来源、种子、匹配占位和统一动作读取。
...
1. 完成第一版剩余硬化：真实脱敏 RSS 夹具、429/退避、并发限制和抓取记录上限。
2. 复核 JSON → SQLite 迁移报告、明文 Passkey 风险和页面/API 脱敏。
3. 停点通过后，再执行 Torra 追更洗版与 RSS 唤醒计划。
4. 全部模拟验收完成后才进入 fnOS 实机窗口。

- SQLite 和种子库第一版已经落地；当前不再把它们列为待编码事项。
- 使用 5～10 个真实脱敏 RSS 样本补齐站点夹具，并完成 429、退避、并发和抓取记录上限。
- 完成临时 SQLite 原子替换式迁移演练和真实 JSON 差异报告；Docker 重启持久化已经通过。
