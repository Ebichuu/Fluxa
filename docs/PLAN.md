...
状态：v2 代码接入与本地验收完成，等待 fnOS 实机窗口
更新时间：2026-07-17
...
- 115、Telegram、HDHive / pansou、Symedia 等网盘业务模块已通过统一设置、订阅开关、任务状态和 `/api/v2` 安全接口接入。
- 网盘候选预览不会返回分享链接或密码；单条转存具备任务链复查、幂等、冷却和审计，默认闸门关闭。
...
- [x] 完成 83 项 Python 回归测试，包括 37 条保留核心接口完整性、v2 网盘闸门/幂等、守卫顺序和 SQLite 标识符安全检查。
...
- [x] 新增 13 条 HTTP v2 契约，接入 115、Telegram、HDHive / pansou 和 MoviePilot。
- [x] 完成全局/订阅级网盘开关、脱敏候选、单条转存、重复复查、幂等、冷却和任务中心状态。
...
手动网盘通道的代码已完成，剩余的是自动执行和实机证据：

- 把 `cloud_allowed` 状态接入后台自动候选执行器。
- 自动执行前再次使用现有重复复查、幂等、冷却和审计服务。
- 真实 115 转存后补充直接状态证据，减少对 Symedia 相邻证据的依赖。
- 自动兜底即使代码完成也继续默认关闭，必须在 PT 主链实机稳定后人工开启。
...
- 控制室：Emby、qB、Torra、Symedia 核心状态，以及 115、Telegram、HDHive / pansou、MoviePilot 辅助状态。
- 任务中心：订阅 → 获取下载 → 进入 115 → 入库四步证据链，并显示 PT / 网盘支线策略状态。
- 日历：订阅播出与入库进度。
- 内容发现：TMDB、JustWatch、豆瓣、腾讯、优酷、爱奇艺、芒果和全球日播。
- 我的订阅：唯一 NasEmby 台账的订阅管理。
- 订阅设置：PT 主通道、NasEmby 来源、网盘开关、等待阈值和资源规则。
- 系统设置：服务连接、115 检查、Telegram 登录/频道、HDHive 授权/签到和系统级安全状态。
...
1. 在 fnOS 部署单容器并只读运行。
2. 配置持久目录和外部服务连接，保持全部写入、管理、搜索和转存闸门关闭。
