...
> 历史迁移设计。当前产品边界由 `docs/superpowers/specs/2026-07-17-pt-primary-control-center-design.md` 覆盖：允许调整顶部导航和媒体抽屉，Telegram 网盘页面延期。
...
当前 Fluxa 仓库成为后续唯一开发主目录，包含最新代码、运行资源、测试、部署文件、接口调用依据和有效资料。v2 使用全新 Git 历史，不复制旧仓库的 `.git`。
...
- [ ] 在任务中心补充网盘支线证据；当前 PT 收口版只完成主链，网盘支线源码保留并延期。
- [ ] 自动云盘兜底代码完成后仍保持默认关闭。
