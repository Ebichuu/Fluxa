...
Torra 秒传已经成功但 CloudDrive2 没有产生可供 Symedia 实时监听的创建事件时，可在“设置 → 自动化”开启“秒传单文件自动入库”并重启。该能力还要求总写闸门已开启；首次运行只记录当前 Torra job 水位，不处理启用前的历史文件。Torra 未带时区的任务时间按 fnOS 北京时间解释后统一转换为 UTC，禁止直接把本地时间当作 UTC 推进水位。以后只把新成功秒传 job 对应的单个媒体文件交给唯一匹配的 Symedia 归档任务，不得用它替代 115 全目录扫描。回滚时关闭该开关并重启即可，已有文件和归档历史不会删除。
