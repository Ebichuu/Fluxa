...
| `GET /api/v2/tasks/summary` | 返回唯一任务链数量、健康/身份/执行三维状态数量、兼容四态 `userCounts`、新派生六态 `outcomeCounts`、阶段数量、服务状态和稳定 `version`；支持 ETag 条件读取；可选 `systemIssues` 返回系统级问题（如 Torra 秒传）的分类级安全摘要；`ledger.transientEventCleanup` 只返回迁移 ID、状态、删除总数和按阶段统计，不公开数据库或备份路径 |
| `GET /api/v2/tasks/chains` | 支持 `healthState`、可重复 `identityState`、`executionState`、可重复 `outcomeState`、兼容 `userState`、`completedDate`、独立 `archivedDate`、`qbActive=1`、`chainId`、`targetKey`、`subscriptionId`、`tmdbId`、`title`、`seasonNumber`、`updatedAfter`、`offset`、`limit` 和 `refresh=1`；`qbActive=1` 按每条链公开 `qbControl.active` 在分页前筛选当前 `downloading/stalled` qB 任务，不附加媒体结果条件并保留 orphan qB 链，其他值返回 `400 TASK_QB_ACTIVE_FILTER_INVALID`；`archivedDate` 按 Asia/Shanghai 从持久台账和当前快照合并 Symedia 文件级成功事件，只有具备明确 `eventAt` 的文件结果才计入，旧事件不得以 `observedAt` 代替发生时间；返回 `archiveSummary`，有历史时不依赖 Symedia 当前在线，无历史且数据源不可读为 `502 TASK_ARCHIVE_SOURCE_UNAVAILABLE`；非法日期为 `400 TASK_ARCHIVED_DATE_INVALID`；默认返回 20 条唯一链路摘要和稳定分页字段；摘要顶层返回 `outcomeState/playableAt`、`pipelineOutcome` 与 `confirmedStageCount`，兼容 `userState/resultText/completedAt/primaryAction` 只由新结果单向投影；可选 `systemIssues` 同任务 summary |
