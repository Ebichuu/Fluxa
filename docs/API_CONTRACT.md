...
| `GET /api/v2/tasks/chains` | 支持 `healthState`、可重复 `identityState`、`executionState`、可重复 `outcomeState`、兼容 `userState`、`completedDate`、独立 `archivedDate`、`chainId`、`targetKey`、`subscriptionId`、`tmdbId`、`title`、`seasonNumber`、`updatedAfter`、`offset`、`limit` 和 `refresh=1`；`archivedDate` 按 Asia/Shanghai 从持久台账和当前快照合并 Symedia 文件级成功事件，返回 `archiveSummary`，有历史时不依赖 Symedia 当前在线，无历史且数据源不可读为 `502 TASK_ARCHIVE_SOURCE_UNAVAILABLE`；非法日期为 `400 TASK_ARCHIVED_DATE_INVALID`；默认返回 20 条唯一链路摘要和稳定分页字段；摘要顶层返回 `outcomeState/playableAt`、`pipelineOutcome` 与 `confirmedStageCount`，兼容 `userState/resultText/completedAt/primaryAction` 只由新结果单向投影；可选 `systemIssues` 同任务 summary |
