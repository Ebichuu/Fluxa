...
新增能力使用真正的 URL 版本契约：`docs/contracts/http-api-contract-v2.json`，当前共 37 条。v1 的 47 条冻结路径和历史状态码不变。
...
当前 47 条 v1 契约不承担新增语义。37 条 `/api/v2` 接口包括：
...
- `GET /api/v2/settings/runtime`：返回全部应用级配置目录；密码、Token、Cookie、API Key 只返回 `hasValue`，不返回明文。
...
以下接口已在 2026-07-18 阶段 6 注册，并计入当前 37 条机器契约：
...
以下接口已经进入 37 条 v2 机器契约和当前 React：
