...
| `GET /api/v2/rss-matches` | 可选 `status`、`limit`、`offset`；默认保持原 `items` 分页。可选 `view=groups` 改为按订阅季集返回 `groups` 分页，配合可选 `groupState` 和 `groupScope=scoreable|cleanup` 筛选组状态/范围，并可选返回全库 `counts`；整组均因 `subscription_missing` 阻断时派生为 `needs_cleanup`，评分主范围排除、待整理范围只返回该类孤立组；每组携带脱敏候选、可靠基线和唯一冠军投影，不触发 Torra 搜索或下载 |
...
私人 RSS 和下载地址按用户选择在 SQLite 中明文保存，但所有读取响应、错误和日志都不得返回完整地址或 Passkey。来源创建返回 `201 + Location`，删除成功返回 `204`；测试返回 `202`、动作 ID 和统一动作轮询 `Location`。来源和种子列表分页，重复来源返回 `409`，非法 RSS/周期/保留期、`identityStatus`、`reviewState` 或 `groupState` 返回 `422`，收集闸门关闭返回 `503`。RSS 收集闸门关闭只阻止测试和后台外部访问，本地来源配置 CRUD 不产生网络请求。

`GET /api/v2/rss-items` 保留原有查询行为，并接受可选 `identityStatus=identified|conflict|unidentified`、`reviewState=needs_review|follow_needs_review|unlinked`、`publishedDate=YYYY-MM-DD`、`tmdbId`、`mediaType`、`seasonNumber` 和 `year` 筛选。旧 `needs_review` 继续返回全库身份未识别或冲突、媒体类型缺失，或电视剧季号缺失的条目；`follow_needs_review` 在相同条件上进一步要求存在 `candidate/triggered/confirmed` 当前追更关系；`unlinked` 只返回不存在上述关系的中性资源。列表与详情可选返回 `followState=linked|unlinked`。`publishedDate` 按 Asia/Shanghai 的自然日筛选 `publishedAt`，缺少上游发布时间时使用本地 `createdAt`，非法日期返回 `422 RSS_QUERY_INVALID`。GET 仍为只读，不执行身份回填。订阅目标搜索优先返回同一 TMDB 身份；电视剧标题回退不强制年份，明确不同季号仍会排除，季号缺失但没有冲突时只作为人工候选返回，并附带 `matchMethod=title_media_scope`、`matchConfidence=fallback` 和 `seasonScopeState=unknown`；电影标题回退仍必须匹配年份。此类回退候选不能触发自动下载或反向认领身份。
