...
## 12. 计划中的私人 PT RSS 种子库接口

以下接口来自 2026-07-18 私人 RSS 种子库设计，尚未实现，也不计入当前机器契约数量：
...
私人 RSS 和下载地址按用户选择在 SQLite 中明文保存，但所有读取响应、错误和日志都不得返回完整地址或 Passkey。来源创建返回 `201 + Location`，删除成功返回 `204`；测试与 Torra 分析返回 `202`、动作 ID 和统一动作轮询 `Location`。来源、种子和匹配列表必须分页。重复来源返回 `409`，非法 RSS/周期/保留期返回 `422`，限频返回 `429`，上游失败返回 `502`；RSS 收集闸门关闭只阻止测试和后台外部访问，本地来源配置 CRUD 不产生网络请求。错误不能包装在 `200` 中。详细边界见 `docs/superpowers/specs/2026-07-18-private-pt-rss-seed-library-design.md`。
