...
| `GET /api/v2/rss-matches` | 可选 `status`、`limit`、`offset`；默认保持原 `items` 分页。可选 `view=groups` 改为按订阅季集返回 `groups` 分页，配合可选 `groupState` 筛选组状态，并可选返回全库 `counts`；每组携带脱敏候选、可靠基线和唯一冠军投影，不触发 Torra 搜索或下载 |
