...
状态：延期；阶段 A-C 底层代码保留，当前 React 不展示，阶段 D 不开发
更新时间：2026-07-17

当前决策：先跑通“媒体控制中心 → Torra → qB → Torra 秒传 115 → Symedia → Emby”。本文件中的 Telegram、HDHive / pansou、影巢、115 分享转存和自动兜底均属于以后版本，不是当前下一项代码主线。

## 1. 目标

未来如重新启用，在媒体控制中心内管理 NasEmby 已有的网盘搜索、115 分享转存、Telegram 频道和 HDHive / pansou 能力。Symedia 继续只负责 115 后整理入库，不接收重复订阅推送。用户仍只使用一套 React 页面和一份 NasEmby 订阅台账。
...
以下接口已经实现并冻结在 `docs/contracts/http-api-contract-v2.json`：

- `GET /api/v2/integrations`
- `GET /api/v2/acquisition/cloud/candidates`
- `POST /api/v2/acquisition/cloud/transfers`
- `PATCH /api/v2/subscriptions/:id/cloud-policy`
- 115、Telegram 和 HDHive / pansou 的细分状态与管理接口

实现使用会话认证、Origin 校验、细分写闸门、候选短期 ID、幂等、冷却、重复复查和脱敏响应。原 NasEmby 业务函数继续作为唯一底层实现。
...
### 阶段 A：只读状态与契约（底层保留，当前页面隐藏）

- [x] 梳理 NasEmby 现有函数输入输出。
- [x] 冻结网盘来源、账号状态和候选预览契约。
- [x] 使用模拟响应实现 Python 只读 API 和测试。

### 阶段 B：页面与手动预览（底层保留，当前无 React 入口）

- [x] 增加订阅开关和系统设置状态卡。
- [x] 曾完成任务中心网盘支线状态和候选计数；当前 PT 收口版本已从 React 隐藏。
- [x] 增加脱敏候选预览。

### 阶段 C：单条安全转存（底层保留，当前无 React 入口）

- [x] 增加幂等任务、确认、复查、冷却和审计。
- [x] 自动测试只使用临时目录和模拟 115 / Symedia。
- [ ] 真实转存等待用户指定实机测试条目。

### 阶段 D：自动兜底（以后版本，当前不开发）

- [x] 实现等待阈值和 `cloud_allowed` 状态计算。
- [ ] PT 主链稳定后接入后台自动执行器。
...
- 以后恢复网盘能力时不得破坏当时的影院大厅、导航和媒体抽屉交互。
