...
class PrivateRssRepository:
    ...
    def initialize(self):
        with self.runtime.transaction(immediate=True) as connection:
            ...
            connection.execute(
                ...
                "subscription_key TEXT NOT NULL, state TEXT NOT NULL DEFAULT 'candidate', reason TEXT NOT NULL DEFAULT '', "
                "created_at TEXT NOT NULL, UNIQUE(item_id, subscription_key))"
            )
    ...
    def upsert_items(self, source_id, items):
        ...
        with self.runtime.transaction(immediate=True) as connection:
            for item in items:
                ...
                if not fingerprint or not str(item.get("title") or "").strip():
                    continue
                ...
                values = (
                    item_id, source_id, fingerprint, str(item.get("guid") or ""), str(item.get("title") or "")[:500],
                    str(item.get("description") or "")[:2000], str(item.get("published_at") or ""), str(item.get("category") or "")[:300],
                    int(item.get("size_bytes") or 0), str(item.get("detail_url") or "")[:4096], str(item.get("download_url") or "")[:4096],
                    str(item.get("media_type") or ""), item.get("season_number"), item.get("episode_start"), item.get("episode_end"),
                    str(item.get("version_summary") or "")[:300], _iso(now), _iso(now), expires,
                )
                connection.execute(
                    "INSERT INTO rss_items (id, source_id, fingerprint, guid, title, description, published_at, category, size_bytes, detail_url, download_url, "
                    "media_type, season_number, episode_start, episode_end, version_summary, created_at, last_seen_at, expires_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                    "ON CONFLICT(source_id, fingerprint) DO UPDATE SET title=excluded.title, description=excluded.description, published_at=excluded.published_at, "
                    "category=excluded.category, size_bytes=excluded.size_bytes, detail_url=excluded.detail_url, download_url=excluded.download_url, "
                    "media_type=excluded.media_type, season_number=excluded.season_number, episode_start=excluded.episode_start, episode_end=excluded.episode_end, "
                    "version_summary=excluded.version_summary, last_seen_at=excluded.last_seen_at, expires_at=excluded.expires_at",
                    values,
                )
    ...
    def record_fetch(self, source_id, status, item_count=0, message="", etag="", last_modified=""):
        source = self.get_source(source_id, public=False)
        if not source:
            return
        now = _now()
        ...
        with self.runtime.transaction(immediate=True) as connection:
            connection.execute(
                "INSERT INTO rss_fetch_runs (source_id, status, item_count, message, created_at) VALUES (?, ?, ?, ?, ?)",
                (source_id, status, int(item_count or 0), safe_message, _iso(now)),
            )
            connection.execute(
                "UPDATE rss_sources SET etag=?, last_modified=?, last_success_at=CASE WHEN ?='success' THEN ? ELSE last_success_at END, "
                "last_error=CASE WHEN ?='success' THEN '' ELSE ? END, next_poll_at=?, updated_at=? WHERE id=?",
                (etag or source["etag"], last_modified or source["last_modified"], status, _iso(now), status, safe_message, next_poll, _iso(now), source_id),
            )
    ...
    def due_sources(self):
        ...
        with closing(self.runtime.connect()) as connection:
            rows = connection.execute(
                "SELECT * FROM rss_sources WHERE enabled=1 AND (next_poll_at='' OR next_poll_at<=?) ORDER BY next_poll_at, name LIMIT 10",
                (now,),
            ).fetchall()
