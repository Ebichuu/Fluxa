...
def write_config(payload: dict[str, object]) -> dict[str, str]:
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    values = dict(DEFAULT_CONFIG)
    current = read_config()
    ...
    for key in CONFIG_FIELDS:
        if key in payload:
            ...
            if key in PRESERVE_EMPTY_FIELDS and not next_value and current.get(key):
                continue
    ...
    for key in CONFIG_FIELDS:
        ...
        os.environ[key] = value
    USER_ENV_PATH.write_text("".join(lines), encoding="utf-8")
    LEGACY_USER_ENV_PATH.write_text("".join(lines), encoding="utf-8")
    return values
