...
- 追更洗版协调器只在 `MCC_TORRA_QUALITY_WATCH_ENABLED=true` 时启动，并继续要求 SQLite 中的追更设置开启；自动 RSS 入口只读 Torra 订阅和权重规则，人工分析及兼容的主动搜索兜底继续受独立闸门约束。
