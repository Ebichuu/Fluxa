...
当前共 332 项回归测试。SQLite、RSS、Torra、MoviePilot 备用、网盘、日历时间线和系统指标测试全部使用临时台账与模拟函数，不连接真实外部服务；覆盖默认闸门、脱敏、原子迁移、Torra 镜像幂等、任务身份/执行状态拆分、正常保护一致性、用户/技术原因分层、首页季集定位、日历未知/逾期判定、RSS 精确搜索与身份回填、追更海报补齐、qB 安全动作和自动化窗口。
