...
- Torra 固定目标推送，以及只读权重规则、追更洗版人工分析、候选下载、job 状态解析、按集 Emby 基准、SQLite 幂等/租约和脱敏审计；自动 RSS 匹配只在 Fluxa 本地影子评分，不触发 Torra 整订阅搜索或下载。
