...
4. 发现和订阅公开兼容层。
5. React 静态目录与 SPA 回退。
6. NasEmby 原 Blueprint，供 `/api/status`、`/api/health` 和源码兼容保留。
7. 保留核心接口隔离守卫。
...
- 浏览器不持有 Emby、qB、Torra、Symedia、TMDB、115、Telegram 或 HDHive 凭据。
