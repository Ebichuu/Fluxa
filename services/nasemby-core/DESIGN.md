...
### 2026-08-10 — 115 到达与 STRM 目标证据补齐

**变更内容**：六阶段事实增加两条保守的目标级只读证据。Torra 没有当前文件的逐文件成功结果时，只有已经归属当前目标且状态成功的 Symedia 记录，其源路径含独立 `115` 路径段，才确认文件已到达 115；文案明确不确认秒传或原始上传方式。Torra 明确文件失败仍优先，只有失败与到达记录均唯一且 Symedia 到达时间严格更晚时才恢复，多文件歧义继续保留失败。Emby TMDB 索引同时记录精确电影和季集目标的 `.strm` 路径；只有当前 movie 或 episode 的 `Path` 明确以 `.strm` 结尾时确认 STRM 播放入口。

**变更理由**：生产链路已经能从当前目标匹配的 Symedia 源记录证明文件来自 115，也能从 Emby 精确目标路径直接观察 `.strm` 文件。此前把这两项一律留为未知会使已完成链路长期显示 4/6；但泛化的 Emby 收录、Series 命中和 Symedia 按日 STRM 数量仍缺少目标级证明，不能放宽。

**影响范围**：Emby 只读索引、115/STRM 六阶段事实、任务链整合测试和维护文档。没有新增外部请求、数据库字段或写动作；路径只在服务端用于精确判定，公开响应不返回原始路径。

### 2026-08-07 — RSS 资源只读媒体元数据与中文搜索

**变更内容**：新增 RSS 媒体元数据解析层，只处理已经具备明确 `media_type + tmdb_id + season` 的已识别资源，依次读取匹配订阅、精确订阅、精确发现候选和现有 discover/TMDB 本地缓存。资源列表与详情可选返回 `mediaTitle/mediaYear/posterUrl`；中文搜索与原 FTS 命中在完整结果集合上合并、去重后再分页。

**变更理由**：原始 PT 发布名适合技术核对，但不能稳定回答实际作品；媒体标题展示后也必须能搜索回同一资源。来源冲突时停止解析，不能通过标题猜测或降级串用其他作品海报。

**影响范围**：Private RSS 只读投影、discover 缓存读取、React 资源中心和对应测试。列表请求不调用 TMDB，不写 RSS、订阅、匹配或外部任务；海报地址去除查询参数和用户信息，并拒绝本地或私网主机。公开响应继续省略订阅 payload、RSS 下载地址、Token、Cookie、Passkey 和内部服务地址。

### 2026-08-07 — RSS 历史范围安全重解析

**变更内容**：新增管理员维护命令 `python -m app.admin repair-rss-scope --preview` 与 `--apply <preview-fingerprint>`。预览只读扫描当前解析器与历史 RSS 范围差异，按重复季号范围、编码误判和其他范围变化聚合返回脱敏计数与指纹；不返回标题、RSS 简介、URL、Passkey 或动作 ID。只有未触发、未确认、未关联外部动作的候选匹配允许自动归档并等待重新匹配，已触发、已确认或外部动作关联的记录进入人工复核。

**变更理由**：解析器修复不能直接在普通启动流程中批量改写历史台账。维护操作必须经过预览、指纹复核、完整 SQLite 备份、`BEGIN IMMEDIATE` 内二次校验和单事务提交，避免历史候选被静默覆盖或重复下发。

**安全边界**：无变更时不创建备份；备份失败、数据漂移、事务或审计失败时整批回滚并记录脱敏失败台账。RSS 原始条目保留，安全候选的旧匹配使用版本化 `rss-scope-repair-v1` 归档标记，普通 `subscription_missing` 清理归档不会重新进入匹配扫描。命令不调用 Torra、qB、115、Symedia 或其他外部写接口，服务启动不会自动执行该维护操作。

### 2026-08-06 — RSS 精准下载与问题事实安全修正

**变更内容**：精准下载和旧 qB 前检统一把 `downloading/stalled/queued/paused` 视为同目标占用，`completed` 继续只作为基线证据而不阻断；10 分钟预览指纹新增当前完整下载地址的 SHA-256 摘要，执行前重新读取 RSS 条目，地址变化会使旧令牌失效。RSS `NxM` 解析在识别季集前剔除独立 `x264/x265/x266` 编码标记，保留 `1x03` 与 `1 x 03`。问题组的阶段、原因码和摘要文案统一取自有效 `pipelineOutcome`，只有缺少有效结果时才回退旧顶层字段。

**变更理由**：排队或暂停的同目标任务仍占用下载所有权，不能允许第二次精准提交；RSS GUID 不保证下载地址及 Passkey 永久不变，令牌必须绑定实际资源但不能公开凭据；编码标记和残留兼容文案都不能冒充季集或最终失败阶段。

**影响范围**：RSS 匹配运行时、私人 RSS 解析器、任务问题组纯投影及对应回归测试。未修改数据库结构、公开 API 或 qB 状态归一化；下载地址及其摘要不进入公开响应、动作摘要和日志，真实外部写闸门与 `automatic` 拒绝保持不变。

### 2026-08-06 — RSS 产物级人工精准 qB 提交

**变更内容**：RSS 候选在保留逐集评分的同时新增 `subscription_key + artifact_key` 产物投影，只有覆盖的全部季集均为唯一冠军时才可执行。产物级预检实时复核 Torra 身份、规则、基线、保存目录、下载器映射和 qB 当前任务，并签发 10 分钟令牌；qB 分类只接受 Torra 明确提供的 `qb_category` 或 `download_category`，缺失时省略该参数，普通媒体 `category`、保存目录、标题和历史任务均不作为分类路由事实。确认接口只接受令牌与幂等键，由后端把 RSS 下载地址和 Torra 订阅路由直接提交到已确认映射的 qB。动作收据按候选事实指纹稳定去重，复用全局单飞、冷却和小时/每日限额；qB 暂未显现时保持 `submitted`，现有质量观察调度只按审计标签恢复已有任务。

**变更理由**：Torra 没有可复用的指定 RSS 资源接口，但 Fluxa 已能从唯一订阅实时取得目录与下载器归属；当前实机订阅响应不提供 qB 分类，而 qB 已有分类属于站点来源，不能由媒体目录机械映射。受限直写 qB 在不猜测分类的前提下保留既有保存目录、mover 和 p115client 验收路径，同时避免把范围包按集重复提交；稳定事实收据和标签恢复消除超时或容器重启后的重复下载窗口。

**影响范围**：私人 RSS 仓储与产物投影、匹配运行时、qB 客户端、自动化动作脱敏、质量观察调度、设置 API、资源中心与回归测试。浏览器不能提交 URL、路径、分类或下载器；公开响应不含 Passkey、内部路径或 Torra 原始 ID。阶段 D 实机链路验收前只允许 `disabled/manual`，`automatic` 继续拒绝。
...
**变更内容**：生产桥接统一解析 Fluxa 本地追更与任务链本轮 Torra 只读追更，使用 Torra ID、TMDB、媒体类型和季号做唯一精确绑定；Torra-only 保持稳定公开键且不写入本地订阅表。qB 对已完成、唯一所有权且标题无集号的剧集季包按 Hash 缓存读取文件清单，水位后的精确 Symedia 集级成功可以直接创建观察单元并建立入库基线。桥接协议升为 v3，v2 收据保留审计但不进入当前摘要。

**变更理由**：任务中心已经能展示 Torra 只读订阅，但旧桥接只读取本地订阅；季包标题也缺少集号，导致明确的 qB 与 Symedia 成功事实被错误归入身份待复核，生产主链无法建立观察基线。

**影响范围**：质量观察订阅解析、任务快照桥接、qB 只读文件清单缓存、基线时间修正、收据摘要和专项测试。标题不参与作品身份匹配；外部读取在 SQLite 事务外完成，不执行历史初始化预览、搜索、下载、推送或归档，自动下载闸门保持关闭。
...
**变更理由**：聚合数量只能证明某日发生过 STRM 活动，不能证明当前任务的播放入口已生成。只凭文件名、归档路径或泛化 Emby 命中会重新引入跨阶段反推；2026-08-10 后只有 Emby 返回当前精确目标且 `Path` 明确以 `.strm` 结尾时，才视为对该播放入口的直接观察。
