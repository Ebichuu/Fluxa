...
首页摘要使用独立后台刷新路径：实时任务链、qB、归档、秒传、追更和 RSS 读取均在 SQLite 事务外完成，七个公开模块用各自短事务写入 `home_summary_module_cache`。同一轮任务链完整快照只读取一次，采集结果按模块分别提交成功或失败；任务链失败不阻止独立 qB、RSS 和追更进度模块保存可靠结果。`GET /api/v2/home/summary` 只读取当前 `global` 与 `Asia/Shanghai` 日期范围，不领取租约、不触发刷新；空缓存立即返回未知结构。进程锁与 `home_summary_refresh_state` 租约共同限制单飞，失败保留上一份完整模块值并降低确认状态；模块与关注项的确认状态按 `unknown > partial > confirmed` 合并，禁止用模块级成功覆盖更保守的事实状态。模块 payload 和错误禁止保存路径、hash、URL、凭据或远端原始 ID。
...
- Torra `secupload_115` 的新版本可在部分 `recent_runs.result` 中提供 `failure_details`、`success_count` 和 `failed_count`，旧版本或部分运行仍可能没有详情。Fluxa 只在本次批次存在结构化详情时展示脱敏文件名、错误分类和可空重试次数；没有详情保持“暂未确认”。文件事实只有通过不可逆完整路径键与当前 qB 路径精确一致时才能绑定媒体链，单凭文件名或分类失败数不得绑定作品。
...
- 47 条冻结 v1 路由和 70 条 v2 路由均在 Python 中存在。
- 当前后端回归基线为 592 项。
- 42 条受保护路由逐条返回 401。
...
### 2026-08-05 — 任务优先首页与追更持久缓存

**变更内容**：首页主结果收敛为今日入库、正在下载、等待自动恢复和需要处理；问题项增加公开影响说明与单一推荐动作，技术名称继续留在任务高级详情。追更工作台新增 SQLite 最后可靠快照、60 秒后台单飞刷新和只排队的刷新请求，GET 不再现场执行 Torra 对账。RSS 首页摘要增加已关联追更的当日命中和需要人工决定计数，站点原始流量继续保留在资源中心。

**变更理由**：避免首页与追更页冷启动被外部读取拖到超时，并让普通页面优先回答结果、影响和下一步，而不是暴露组件技术状态。

**影响范围**：首页与追更公开可选字段、SQLite 缓存表、单例后台线程、资源中心关联统计、React 展示和 81 条 v2 契约。旧工作台字段、六阶段状态、导航、Torra/Symedia 配置和所有外部写权限保持不变；缓存失败保留上一份可靠结果，公开错误不含上游原文。

### 2026-08-04 — 日历持久缓存、对账待办与 RSS 失效归属归档

**变更内容**：日历月快照改为 SQLite 持久缓存与后台单飞刷新，页面 GET 只读最后可靠结果；追更工作台新增独立 `reconciliationActionRequired`；RSS 匹配增加版本化归档状态、有效/失效所有权投影、持久预览、整批确认和审计表。

**变更理由**：避免日历缓存过期后现场重算整月，区分对账问题与媒体处理失败，并让失去订阅归属的孤立候选退出评分主列表而不删除原始 RSS 证据。

**影响范围**：日历缓存仓储/worker、追更与首页只读摘要、私人 RSS 仓储和 v2 API、临时 SQLite 测试及机器契约。RSS 清理仅允许 `blocked + subscription_missing`，确认时整批重验并原子归档；不触发任何外部搜索、下载、秒传、归档或刷新动作。

### 2026-08-02 — 首页后台模块缓存与部分确认统计

**变更内容**：新增七模块 SQLite 首页缓存、后台单飞刷新器和纯读首页 GET；缓存空缺、过期或刷新失败按模块投影未知/部分确认。任务链快照每轮只读一次，qB、RSS 和追更进度分别捕获采集结果；追更条目读取错误计入未确认数量，不再清空已确认缺集数。模块与关注项确认状态只向更保守方向合并。下载完成未入库与追更缺集保留已确认值，并增加互斥的未确认对象计数。候选来源刷新另外返回可选跳过/错误计数，旧订阅总调度明确命名为 Torra 订阅搜索。

**变更理由**：避免首页冷启动现场串行读取全链路导致超时，也避免单个证据缺失把全部可用统计清成未知。

**影响范围**：SQLite schema version 7、首页只读 API、Gunicorn 单例后台线程、React 首页/设置/活动文案与回归测试。旧响应字段和状态码保持兼容，不新增任何外部写动作；Symedia 后六项能力、身份清理和 Torra 策略调整不在本波次。
...
**变更理由**：Torra 当前把 `auto/rss` 作为单次运行参数，没有公开订阅级永久模式字段。把运行任务数或标题当成模式数量会产生错误事实，也无法安全生成批量调整清单。

**影响范围**：Torra 只读客户端、独立搜索策略投影模块、现有摘要可选响应、React 控制室、TypeScript 类型、定向模拟测试和维护文档。没有新增路由或 Torra 写调用；未知模式不推测，内部 job/订阅 ID、目录、下载器、URL、Cookie 和 Passkey 均不公开。

### 2026-07-30 — RSS 跟随模式与定时整订阅搜索止损

**变更内容**：质量观察生命周期默认改为 `follow_rss`，当前订阅策略优先、全局策略兜底并同步观察单元投影；协调器在跟随模式下只维持宽限期和到期状态，不提交新的 Torra 整订阅分析。已有外部 job 继续轮询，未提交的旧调度动作取消；`fixed_window` 作为显式高级兼容模式保留。

**变更理由**：RSS 候选已经在 Fluxa 本地完成订阅绑定和影子评分，继续按 720/1440/2880 分钟调用 Torra 会重复触发全站 PT 搜索，违背搜索降载目标。

**影响范围**：质量观察仓储、策略解析、协调器、订阅自动化设置和既有 v2 设置响应；不新增路由，不修改 Torra，不执行真实分析或下载，人工分析与已有外部任务续查保持可用。

### 2026-07-30 — RSS 订阅级精准下载预检止损

**变更内容**：新增匹配级只读预检，按同一次请求重新读取冠军、严格分差、订阅绑定、Torra 当前规则、候选评分、当前版本基线、qB 活动任务和 Torra 忙碌状态。公开响应只返回匹配 ID、规范目标、脱敏版本摘要、分数和稳定阻断码；空对象以外请求返回 422。

**变更理由**：Torra 正式洗版接口只接受自身分析会话候选，通用下载器入口没有订阅 ID，并绕过订阅去重、分集文件选择、下载快照和目标回写，不能作为 Fluxa RSS 精准下载入口。

**影响范围**：RSS 匹配运行时、1 条增量 v2 路由、机器契约和回归测试。当前预检固定追加 `TORRA_EXACT_RESOURCE_ENDPOINT_UNAVAILABLE`，不创建 provider action，不调用 Torra 分析或下载，不公开 URL、目录、下载器 ID、远端 ID、Cookie 或 Passkey；阶段 D 保持关闭。

### 2026-07-30 — RSS 订阅绑定与 Torra 规则影子评分

**变更内容**：RSS 自动匹配从调用 Torra 整订阅分析改为读取正式只读权重规则并在 Fluxa 本地评分。每个候选保存规范订阅所有者、TMDB 季集范围、单一 `artifactKey`、规则快照、候选分数和稳定决策；E02-E03 范围包可以投影到多个观察单元，但只生成一次评分动作。观察单元创建或基线确认时按首次下载时间定向回扫已到达候选，规则、身份、范围、所有权或评分字段不完整时保存“暂未确认”而不是零分。

**变更理由**：旧自动入口把 RSS 仅作为触发信号，随后让 Torra 重新搜索整条订阅，既没有评价当前 RSS 候选，也不能覆盖首版下载期间提前到达的高质量版本，还会制造额外 PT 搜索压力。

**影响范围**：Torra 只读客户端、RSS/质量观察 SQLite 增量字段与规则快照、自动匹配/回扫运行时、公开可选 DTO、影子评估审计、React 种子库和回归测试。自动路径不调用 Torra 搜索或下载；人工 Torra 分析/下载 API 继续作为明确兜底，旧字段、状态码和请求体保持兼容，浏览器不接收原始 Torra 订阅 ID、路径、URL、Cookie 或 Passkey。

### 2026-07-30 — 统计范围元数据

**变更内容**：首页、任务、追更和日历只读响应兼容增加 `statisticsMeta`，每个统计键固定返回 `scope/unit/observedAt/confirmation`。任务范围为当前唯一任务链，追更范围为当前完整追更台账，日历范围为本次查询，首页分别标记今日媒体结果、当前 qB 快照和当前任务问题组；确认状态固定为 `confirmed/partial/unknown`。

**变更理由**：同名“已可播放”在任务中心和追更页使用不同集合，旧页面没有解释范围；同时上游不可读时兼容零值容易被误认为真实零。

**影响范围**：四个只读聚合器、可选 v2 响应字段、TypeScript 类型、统计展示和回归测试。旧数值、URL、请求参数、HTTP 状态码、任务事实、事件台账和外部写权限均不改变；真实零继续返回零，前端只在 `confirmation=unknown` 时显示未知。

### 2026-07-30 — 媒体最终结果与遗留问题分离

**变更内容**：任务链从六阶段当前事实派生可选 `mediaResult` 和 `residualIssues`。媒体结果只接受目标范围内已验证的最高下游成功；系统分类事实不生成媒体结果。遗留问题只保留不会推翻已确认下游结果的上游明确失败，计划中的自动恢复不计入遗留问题。

**变更理由**：同一媒体已经入库或可播放时，重复 qB 任务、旧下载失败或清理问题不应把最终结果改写成未完成；但这些问题仍需要用户看到并处理。

**影响范围**：任务链派生、任务公开响应、追更工作台、任务卡片、P0.4 回归测试和 v2 可选字段。`pipelineOutcome`、旧状态、筛选、统计、事件台账、六阶段事实和外部写权限保持兼容。
...
**变更理由**：下载器当前有多少活跃任务与媒体链是否需要处理是两个独立事实；沿用媒体结果过滤会让首页显示 0，而控制室和任务详情仍显示真实活跃任务。

**影响范围**：qB 只读客户端、任务链公开增量字段、首页摘要、v2 可选查询、React 深链与统计文案、API/URL 文档和回归测试。旧 `activeDownloadTasks/concurrentDownloadCount`、媒体结果、观察窗、永久事件、URL、HTTP 方法和既有状态码均保持兼容。

### 2026-07-29 — qB 共享评估与控制室统一

**变更内容**：新增无副作用的 qB 单任务评估器和聚合器，任务链与 `/api/qbittorrent/summary.assessment` 共用调用方传入的观察时间、900 秒窗口和状态优先级。摘要只增加可选字段，原始 `counts.stalled`、任务列表、URL、方法和状态码均保持不变；轮询 stalled 超时仍只属于当前事实，不进入永久事件。

**变更理由**：控制室直接使用原始 stalled 会绕过任务链的 15 分钟观察窗，导致同一快照在首页、任务中心和控制室出现不同结论与错误的顶部异常数。

**影响范围**：qB 只读摘要、六阶段 qB 来源事实、公开 v1 契约、TypeScript 消费类型、控制室投影和回归测试。公开原因使用固定脱敏文案，旧消费者可忽略新增字段。

### 2026-07-29 — 问题组统计与 qB 观察窗

**变更内容**：首页新增独立问题组与身份未确认资源计数；可靠媒体分组要求已关联身份、有效 TMDB ID、明确媒体类型和电视剧季号，无身份资源只按机械规范化标题做非持久展示分组。qB 普通 stalled/零速度增加 900 秒观察窗，正速度立即恢复，missing/error 仍立即失败；观察窗状态不写永久历史。

**变更理由**：缺少身份的多个分集不能伪称多部作品，短暂 qB stalled 也不能在一分钟内反复触发红色异常。

**影响范围**：首页摘要可选字段、qB 六阶段事实、资源事件过滤、公开契约与回归测试。旧异常计数字段、任务身份、资源所有权和数据库 schema 均不改变。

### 2026-07-29 — 事实状态与统计说明收口

**变更内容**：Symedia 保护判定收紧为低评分、取消或跳过覆盖、已有更优版本五类稳定规则；混合结果优先保留真实失败。首页在旧资源级计数之外新增作品级与资源级可选计数，作品只按媒体类型、TMDB 身份和电视剧季号聚合。qB 六阶段事实区分下载、等待、无速度、卡住、校验、做种和失败，卡住时只使用 `last_activity` 说明无活动时长。Symedia 今日摘要按稳定 ID 或规范文件路径去重，并用互斥分类保证分项合计等于今日记录。

**变更理由**：识别失败不能被“跳过”关键词误判为正常保护；分集资源数不能冒充受影响作品数；qB 数量摘要不能解释具体阻塞；Symedia 原始重复扫描和重叠保护分类无法直接对账。

**影响范围**：Symedia/qB 只读适配、六阶段事实、首页摘要、控制室公开类型与回归测试。`actionRequired/mediaActionRequired` 保持资源级兼容，只增加可选 `actionRequiredWorks/actionRequiredResources` 和 qB `lastActivity`；没有改变路由、状态码、永久事件白名单或外部写能力。候选调度未启动和 Torra 自动推送关闭继续保持中性。
...
**变更理由**：历史发生时间不能随当前快照的新鲜度过期而丢失，也不能把一次观察时间宣称为真实处理时间；范围资源必须只有一个底层所有者，日历只能投影范围，不能复制或猜测 artifact 归属。

**影响范围**：六阶段事实归一化与来源适配、资源事件兼容加列、公开白名单、集级证据投影和定向回归测试。旧事实缺少新字段时继续兼容，外部写权限、旧 `stages/steps` 响应和现有状态码均未改变。

### 2026-07-28 — Torra 秒传文件详情与恢复计数 P1.2

**变更内容**：Torra 读取层新增独立秒传结果解析器，优先消费 `recent_runs.result.success_count/failed_count`，兼容 `result` 对象/数组和 `failure_details` 数组/对象；旧版本继续回退安全 message 计数。失败详情按不可逆文件键去重，只保留脱敏显示名、错误分类、可空重试次数和公开批次关联。系统问题返回 file-scope `cloud115` facts；仅当完整路径键与当前 qB 保存路径精确一致时才把事实绑定媒体链，否则保持 system-category 未确认。任务中心显示本批文件摘要，无详情时只写“本次运行没有文件级详情”。

**变更理由**：Torra 已提供结构化文件证据，Fluxa 不应继续把某一次没有详情写成永久能力限制；同时文件名和分类计数不足以证明媒体归属，必须防止全局秒传失败污染所有任务。结构化计数也必须优先于可能变化的中文 message 文本。

**影响范围**：Torra 只读适配、秒传系统问题、115 文件事实、公开 presenter、任务中心类型/样式和回归测试。API 只增加可选响应字段；旧字段、状态码、手动重试幂等/锁/写闸门保持不变。路径仅参与本地不可逆关联，不进入响应；错误原文、凭据、run/config/task ID 均不公开。

### 2026-07-28 — Symedia 能力证据与洗版摘要 P1.1

**变更内容**：`/api/symedia/summary` 在兼容原统计字段的基础上新增七项独立能力证据。当前只有已验证的 transfer history 请求可返回 `available`；归档监控、云盘消息监听、Webhook、STRM 生成、归档调度和文件观察没有正式读取接口时统一返回 `unknown + NOT_INTEGRATED`。新增共享 Symedia 证据归一化器，兼容真实响应的布尔与 `0/1` 状态；缺失状态保持证据不足，不再误算成功。洗版摘要分别返回可证明的成功替换、低分保护、取消覆盖、真实失败和脱敏最近对象。

**变更理由**：历史接口可读只能证明归档记录可查询，不能证明 Symedia 全部后台服务在线；真实 `0/1` 状态若继续使用 Python 身份比较，会把失败记录误计为成功并污染任务事实。洗版结果也必须只从明确状态、保护原因和源/目标评分证据派生。

**影响范围**：Symedia 只读摘要、任务六阶段来源适配、保护原因解析、控制室公开类型和回归测试。响应只增加可选字段，旧字段由同一归一化结果投影；没有新增外部请求、写动作或 STRM 成功推断，路径和错误原文不进入洗版摘要。

### 2026-07-28 — 历史追更安全分类与迁移 P0.5c

**变更内容**：新增历史追更四类只读预览、SQLite 版本化备份、最新指纹复核、单事务候选 upsert/旧追更删除/迁移记录与内部补偿清单，以及脱敏运行查询。订阅设置页只在用户主动预览后展示分类，执行要求确认短语、幂等键和当前预览指纹。

**变更理由**：不能把所有 `only_fluxa` 一律删除；人工追更、已有 Torra/resource 下游归属和证据不明记录必须保留。只有明确 auto/source 来源、身份和季号完整且没有下游归属的行可以迁回候选池。

**影响范围**：订阅仓库、候选迁移运行时、Flask 装配、订阅设置页、样式、TypeScript API、70 条 v2 契约和临时 SQLite 回归。迁移不调用 Torra、qB、115、Symedia 或 Emby，真实数据仍只允许先运行预览。
