...
首页摘要使用独立后台刷新路径：实时任务链、qB、归档、秒传、追更和 RSS 读取均在 SQLite 事务外完成，七个公开模块用各自短事务写入 `home_summary_module_cache`。`GET /api/v2/home/summary` 只读取当前 `global` 与 `Asia/Shanghai` 日期范围，不领取租约、不触发刷新；空缓存立即返回未知结构。进程锁与 `home_summary_refresh_state` 租约共同限制单飞，失败保留上一份完整模块值并降低确认状态，模块 payload 和错误禁止保存路径、hash、URL、凭据或远端原始 ID。
...
**变更内容**：新增七模块 SQLite 首页缓存、后台单飞刷新器和纯读首页 GET；缓存空缺、过期或刷新失败按模块投影未知/部分确认。下载完成未入库与追更缺集保留已确认值，并增加互斥的未确认对象计数。候选来源刷新另外返回可选跳过/错误计数，旧订阅总调度明确命名为 Torra 订阅搜索。
