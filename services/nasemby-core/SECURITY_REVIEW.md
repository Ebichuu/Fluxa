...
- Torra 目标固定在服务端，浏览器只能提交确认和 12–128 字符幂等键；服务端重新读取唯一台账并复核分类、保存路径、下载器 ID 和在线查重。
- 首次推送、相同幂等键回放和同订阅 60 秒冷却均由模拟客户端覆盖；失败只返回固定机器码和脱敏摘要，不回显上游异常或凭据。
- 系统指标只返回 CPU、内存、磁盘和网络白名单字段，并使用 30 秒缓存；内部路径、Emby 库列表和 Token 已由测试确认不会外泄。
- 自动测试中的活动日志全部重定向到临时目录；完整 85 项回归前后真实活动日志文件未发生变化。
- 本轮扫描结果：生产 Python 0 Critical / 0 High，React 0 告警，npm 生产依赖 0 漏洞；2 Medium / 1 Low 为既有延期源码和兼容显示哈希提示。
- 最终镜像 `media-control-center:v2-pt-final` 使用临时内存持久目录完成只读冒烟：会话认证有效，订阅写入和 Torra 推送返回 403，保留核心接口返回 503；运行层只有 Python/Gunicorn，不含 Node/npm。
